...
const optionMatches = (option: SelectItemType, value: string) => {
  ...
  return normalizeNetworkValue(option.value) === normalizedValue || normalizeNetworkValue(option.label) === normalizedValue
}
...
const findProvinceParent = (provinceValue: string) => {
  ...
  for (const [country, provinces] of Object.entries(provincesByCountry)) {
    const province = provinces.find((item) => optionMatches(item, normalizedProvince))

    if (province) return { country, province: province.value }
  }
...
const findCityParent = (cityValue: string) => {
  ...
  for (const [country, provinces] of Object.entries(citiesByCountryAndProvince)) {
    for (const [province, cities] of Object.entries(provinces)) {
      const city = cities.find((item) => optionMatches(item, normalizedCity))

      if (city) return { country, province, city: city.value }
    }
  }
...
const findCategoryParent = (categoryValue: string, preferredType?: string) => {
  const preferredOptions = preferredType ? getCategoryOptions(preferredType) : []
  const preferredCategory = preferredOptions.find((category) => optionMatches(category, categoryValue))

  if (preferredType && preferredCategory) return { type: preferredType, category: preferredCategory.value }

  for (const type of profileTypesList.map((item) => item.value)) {
    const category = getCategoryOptions(type).find((item) => optionMatches(item, categoryValue))

    if (category) return { type, category: category.value }
  }
...
const getSubCategoryOptions = (selectedType: string, selectedCategory: string) => {
  ...
  return uniqueOptions(Object.values(subCategoriesByType).flatMap((categoryMap) => Object.values(categoryMap).flat()))
}

const findSubCategoryParent = (subCategoryValue: string, preferredType?: string, preferredCategory?: string) => {
  const preferredOptions = getSubCategoryOptions(preferredType ?? '', preferredCategory ?? '')
  const preferredSubCategory = preferredOptions.find((subCategory) => optionMatches(subCategory, subCategoryValue))

  if (preferredType && preferredCategory && preferredSubCategory) {
    return { type: preferredType, category: preferredCategory, subCategory: preferredSubCategory.value }
  }

  const types = preferredType ? [preferredType] : profileTypesList.map((item) => item.value)

  for (const type of types) {
    const categoriesByType = subCategoriesByType[type] ?? {}

    for (const [category, subCategories] of Object.entries(categoriesByType)) {
      const subCategory = subCategories.find((item) => optionMatches(item, subCategoryValue))

      if (subCategory) return { type, category, subCategory: subCategory.value }
    }
  }
...
const filterKeys: NetworkFilterKey[] = ['subCategory', 'category', 'type', 'city', 'province', 'country']

export const getNetworkFiltersFromParams = (params: Partial<Record<NetworkFilterKey | 'tag', string>>): NetworkFilters => {
  ...
  const type = params.type && profileTypesList.find((item) => optionMatches(item, params.type ?? ''))
  ...
  if (params.subCategory) {
    const subCategoryParent = findSubCategoryParent(params.subCategory, filters.type, filters.category)
...
export const getNetworkFiltersFromTag = (tag: string, currentFilters: NetworkFilters = defaultNetworkFilters): Partial<NetworkFilters> => {
  const country = findCountryByValue(tag)
  if (country) return { country: country.value }

  const provinceParent = findProvinceParent(tag)
  if (provinceParent) return { country: provinceParent.country, province: provinceParent.province }

  const cityParent = findCityParent(tag)
  if (cityParent) return { country: cityParent.country, province: cityParent.province, city: cityParent.city }
  ...
  if (categoryParent) return { type: categoryParent.type, category: categoryParent.category }

  const subCategoryParent = findSubCategoryParent(tag, currentFilters.type, currentFilters.category)
  if (subCategoryParent) {
    return {
...
export const getNetworkTagHref = (tag: string) => {
  ...
  if (entry) return `/network?${entry}=${encodeURIComponent(filters[entry] ?? '')}`
...
export const FilterSidebar: React.FC<FilterSidebarProps> = ({
  ...
  const cityOptions =
    selectedCountry && selectedProvince
      ...
      : selectedCountry
        ? uniqueOptions(Object.values(citiesByCountryAndProvince[selectedCountry as CountryCode] ?? {}).flat())
        : uniqueOptions(Object.values(citiesByCountryAndProvince).flatMap((provinceMap) => Object.values(provinceMap).flat()))

  const categoryOptions = getCategoryOptions(selectedType)
  const subCategoryOptions = getSubCategoryOptions(selectedType, selectedCategory)
  ...
  const handleSubCategoryChange = (value: string) => {
    const subCategoryParent = findSubCategoryParent(value, selectedType, selectedCategory)
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <Select
            ...
            placeholder="Selecione o paÃƒÂ­s"
        ...
        <div className="flex flex-col gap-2">
          ...
          <Select
            ...
            placeholder={
              selectedCountry
                ? 'Selecione a provÃƒÂ­ncia'
                : 'Selecione o paÃƒÂ­s primeiro'
            }
          ...
          <Select
            ...
            placeholder={
              selectedProvince
                ...
                : 'Selecione a provÃƒÂ­ncia primeiro'
            }
            options={cityOptions}
            triggerClassName="border-[1.3px] border-white px-3 text-rede-white outline-none"
            popoverClassName="rounded-[8px] border-[1.3px] border-white px-3 text-rede-white outline-none mt-[10px]"
        ...
        <div className="flex flex-col gap-2">
          ...
          <Select
            ...
            placeholder="Selecione a profissÃƒÂ£o"
